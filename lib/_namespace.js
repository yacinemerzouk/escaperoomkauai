...
Bolt.Collections.tikiCountdownStatus = new Mongo.Collection('tikiCountdownStatus',{idGeneration:'STRING'});
Bolt.Collections.lostContinentCountdownStatus = new Mongo.Collection('lostContinentCountdownStatus',{idGeneration:'STRING'});
Bolt.Collections.seanceCountdownStatus = new Mongo.Collection('seanceCountdownStatus',{idGeneration:'STRING'});
Bolt.Collections.SuccessRates = new Mongo.Collection('successRates',{idGeneration:'STRING'});
...
if( Meteor.isServer ) {
    Meteor.startup(function () {
        Bolt.Collections.Games._ensureIndex({"date": 1});
    });
}
...
/**
 * TIKI COUNTDOWN COLLECTION RULES
 */
Bolt.Collections.tikiCountdownStatus.allow({
    insert: function( userId, doc ){
        // return userId ? true : false;
        return true;
    },
...
/**
 * LOST CONTINENT COUNTDOWN COLLECTION RULES
 */
Bolt.Collections.lostContinentCountdownStatus.allow({
    insert: function( userId, doc ){
        // return userId ? true : false;
        return true;
    },
...
/**
 * SEANCE COUNTDOWN COLLECTION RULES
 */
Bolt.Collections.seanceCountdownStatus.allow({
    insert: function( userId, doc ){
        // return userId ? true : false;
        return true;
    },
...
Bolt.getCalendarDay = function( date ){
    ...
    day.games = Bolt.Collections.Games.find({date:day.date},{sort:{time:1}}).fetch();
...
if( Meteor.isClient ) {
    ...
    Bolt.getConfirmationEmailBody = function( reservation ){
        ...
        var body = '<div style="background-color: #ddd; padding: 30px;">' +
            ...
            '<b>CANCELLATIONS</b>' +
            '<br><br>' +
            'Cancellations made more than 72 hours before the scheduled game start time are refunded in full. For other cancellations a cancellation fee of $70 will be charged.' +
            '<br><br>' +
            '<b>PARKING</b>' +
            '<br><br>' +
            ...
            '<br><br>' +
            '* Our games may include sudden changes in lighting. If a member of you party has epilepsy, please contact us.' +
            '<br><br>' +
            'See you soon!' +
            '<br><br>' +
            '---' +
     ...
     * @returns String : the email body HTML
     */
    Bolt.getCoronavirusEmailBody = function(){
        ...
        var body = '<div style="background-color: #ddd; padding: 30px;">' +
            ...
            '<br><br>' +
            '<div style="background-color: #fff; padding: 30px; max-width: 380px; margin: 0 auto; ">' +
            'Hello and thank you for signing up for a game!' +
            '<br><br>' +
            'Safety is our top priority right now. Please review our guidelines:' +
            '<br><br>' +
            '- Please make sure to bring your face mask and wear it into the lobby and maintain a distance from your game master.' +
            '<br><br>' +
            '- Our credit card processor has a pay-by-phone with tap feature.  We encourage you to use it!' +
            '<br><br>' +
            '- Once the game starts, you may choose to remove your mask.' +
            '<br><br>' +
            '- Please put your mask back on once your game ends.' +
            '<br><br>' +
            'We have sinks and sanitizer for you after the game!' +
            '<br><br>' +
            'If you arrive late we may have to cut your game time short.' +
            '<br><br>' +
    ...
    Meteor.startup(function () {
        ...
        sitemaps.add('/pages.xml', function () {
            ...
            _.each(Router.routes, function (route) {
                ...
                if (route && route.options && route.options.sitemap && route.options.name != 'room' ) {

                    // console.log( 'CHECKINT ROUTES', route.path(), route.options );
...
Bolt.getSuccessRates = function(){

    if( Meteor.isClient ) {

        // console.log( 'GETTING SUCCESS RATES' );

        if( ! Session.get( 'successRates' ) ) {

            // console.log( 'DONT HAVE RATES YET' );

            Meteor.call( 'getSuccessRates', function( error, response ){

                // console.log( 'BACK FROM METHOD', error, response );
                if( error ){
                    // console.log( 'GET SUCCESS RATES ERROR', error.message );
                }else{
                    // console.log( 'SETTING RATES IN SESSION', response );
                    Session.set( 'successRates', response  );
                }
            });
